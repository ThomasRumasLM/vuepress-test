...
<#macro imageFilter config >
    ...
    <ul class="kl-showmore kl-visual-filter js-form-field-list" id="js-showMore-${config.filter.code}" data-nb-filters-to-display="${nbValuesToDisplay}">
    ...
    <#assign mediaUrl = (filter.media.url)!"" >
     ...
     "label" : "${filter.value} <span class='kl-visual-filter__label-quantity' aria-hidden='true'>(${total})</span>",
     ...
     "CSSClass" : {
     "label" : "kl-visual-filter__label"
     },
     "reversed" : true,
     "img" : "${mediaUrl}"
     }>
     <li class="ko-filter-item__image js-showmore-elem" <@macros.cerberus "ELEM_filtrevisuel" />><@fields.checkboxImage optionFilterImg /></li>

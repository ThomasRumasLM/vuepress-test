...
<select class="js-select-sorting" id="sorting-select">
...
describe('Tracking class', () => {
  test('If Class Tracking Form is defined', () => {
...
describe('Tracking type sorting ',() => {
  const select  = document.querySelector('.js-select-sorting');
  ...
  test('If type of sorting change', () => {
    const evnt  = eventFire(select, 'change');
    const testTracking  = new Tracking({event:evnt,type:'sorting'});
    debugger;
    expect(testTracking.trackingConfigObject).toEqual({'action': 'Electricité et domotique/Alarme maison, caméra de surveillance et détecteur de fumée/Extincteur/', 'categorie': 'utilisation tri', 'label': 'Pertinence'});
  });

});

describe('Tracking type filter ',() => {
  ...
  test('If select filter', () => {
    const evnt  =  eventFire(filters[0], 'click');
    const testTracking  = new Tracking({event:evnt,type:'filter'});
    expect(testTracking.trackingConfigObject).toEqual({'action': 'Electricité et domotique/Alarme maison, caméra de surveillance et détecteur de fumée/Extincteur/', 'categorie': 'facette', 'label': 'Type de produit', 'parameters': {'dimension20': 'Poteau'}});
  });
